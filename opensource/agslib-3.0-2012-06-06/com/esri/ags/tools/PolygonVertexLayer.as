package com.esri.ags.tools
{
    ...
    public class PolygonVertexLayer extends EventDispatcher
    {
        private var _vertexDragInProgress:Boolean = false;
        private var _tool:EditTool;
        private var _map:Map;
        private var _childrenPerRing:Array;
        private var _vertexIndex:int;
        private var _ringIndex:int;
        ...
        public function PolygonVertexLayer()
        {
            this.arrGraphics = [];
            return;
        }// end function
        ...
        private function vertex_mouseOver(event:MouseEvent) : void
        {
            ...
            this._selectedVertexStartGeometry = this._map.toMapFromStage(event.stageX, event.stageY);
            this._vertexIndex = this.selectedVertex.attributes.index;
            this._ringIndex = this.selectedVertex.attributes.ringIndex;
            this._childrenPerRing = [];
            for each (_loc_2 in this.arrGraphics)
            {
                
                if (_loc_2.attributes.ringIndex == this._ringIndex)
                {
                    this._childrenPerRing.push(_loc_2);
                }
            }
            if (this._tool.allowDeleteVertices)
            {
            }
            if (this._childrenPerRing.length > 3)
            {
        ...
        {
            dispatchEvent(new EditEvent(EditEvent.CONTEXT_MENU_SELECT, event.target as ContextMenu, null, this._vertexIndex, 0, this._ringIndex));
            return;
        }// end function
        ...
        public function load(polygon:Polygon) : void
        {
            var _loc_4:int = 0;
            this.clear();
            var _loc_2:Array = [];
            var _loc_3:int = 0;
            while (_loc_3 < polygon.rings.length)
            {
                
                _loc_2 = polygon.rings[_loc_3];
                if (_loc_2[(_loc_2.length - 1)].x == _loc_2[0].x)
                {
                }
                if (_loc_2[(_loc_2.length - 1)].y != _loc_2[0].y)
                {
                    _loc_2.push(_loc_2[0]);
                }
                _loc_4 = 0;
        ...
        private function addGraphic(pt:MapPoint, index:int, ringIndex:int, symbol:Symbol) : void
        {
            var _loc_5:Graphic = null;
            _loc_5 = new Graphic(pt, symbol);
            _loc_5.attributes = {index:index, ringIndex:ringIndex};
